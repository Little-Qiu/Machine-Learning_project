...
# Step 3: 加载预训练的VGG16模型
#vgg16 = models.vgg16(weights=VGG16_Weights.IMAGENET1K_V1)  # 使用枚举类型加载预训练权重
vgg16 = models.resnet18(weights='IMAGENET1K_V1')
# 移除VGG16的分类头，保留特征提取部分
...
def extract_features(data_loader, model, device):
    ...
    with torch.no_grad():  # 不需要计算梯度
        for inputs, targets in data_loader:
            ...
            # 将特征从(64, 512, 7, 7)展平为(64, 512*7*7)
            x = x.view(x.size(0), -1)  # 展平操作
            features.append(x.cpu().numpy())  # 转换为numpy数组（迁移到CPU）
            labels.append(targets.cpu().numpy())  # 标签（迁移到CPU）

    return np.concatenate(features, axis=0), np.concatenate(labels, axis=0)


# 提取训练集和测试集的特征
X_train, y_train = extract_features(trainloader, vgg16_features, device)
X_test, y_test = extract_features(testloader, vgg16_features, device)

# Step 5: 训练随机森林分类器
rf_classifier = RandomForestClassifier(n_estimators=100, random_state=42)

# Step 6: 记录训练过程中的准确率和损失
train_accuracies = []
val_accuracies = []
